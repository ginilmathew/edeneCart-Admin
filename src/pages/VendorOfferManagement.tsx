    ...
    { 
      ...
      render: (row: Product) => (
        <span className="inline-flex items-center rounded-full bg-surface-muted px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider text-text-muted border border-border">
          {row.categoryEntity?.name || "Uncategorized"}
        </span>
      )
    },
    { 
    ...
    <div className="space-y-6">
      <Card padding="none">
        <div className="p-4 md:p-6 border-b border-border/60">
          <CardHeader 
            title="My Product Offers" 
            subtitle="Configure discounts and bulk offers for your products"
...
export function OfferEditModal({ 
  ...
  const productOffers = useMemo(() => allOffers.filter(o => o.productId === productId), [allOffers, productId]);
  
  const [createOffer] = useCreateVendorPortalOfferMutation();
  const [updateOffer] = useUpdateVendorPortalOfferMutation();
  const [deleteOffer] = useDeleteVendorPortalOfferMutation();
      ...
      <div className="space-y-8 py-2">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-text-muted/40">{productOffers.length} Offers</span>
          </div>
